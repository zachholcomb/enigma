...
class ShiftGeneratorTest < Minitest::Test
  ...
  def test_it_can_generate_random_five_digit_number_key
    assert_instance_of String, @shift_generator.key_gen.generate_random_key
    assert_equal 5, @shift_generator.key_gen.generate_random_key.to_s.length
  end
  ...
  def test_it_can_split_return_value_of_generate_random_key_into_separate_keys
    ...
    random_test_key = @shift_generator.key_gen.generate_random_key
    assert_instance_of Array, @shift_generator.key_gen.create_key(random_test_key)
    assert_equal 4, @shift_generator.key_gen.create_key(random_test_key).length
  end
